...
if ($opc_cn == -1) {
    $tabla = $fn155->fn155_rusuario_all();
    $include = 1;
}
...
if ($opc_cn == 1) {
    ...
    $estado = 0;
    $cuenta = $fn155->fn155_cusuario_xdata($nombre_usuario, $apellido_usuario, $telefono_usuario, $email_usuario, $cedula_usuario,$estado);
...
if ($opc_cn == 2) {
    ...
    $cuenta = $fn155->fn155_uusuario_x($nombre_usuario, $apellido_usuario, $telefono_usuario, $email_usuario, $cedula_usuario,$id);
...
if ($opc_cn == 3) {
    ...
    if (move_uploaded_file($_FILES['dato_5']['tmp_name'], $fichero_subido)) {
        $cuenta = $fn155->fn155_uavisos_ximg($id, $dato);
        echo $fnalert->fnalert_edit($cuenta);
    } else {
...
if ($opc_cn == 4) {
    $id = $_POST['dato_1'];
    $estado = $_POST['dato_2'];
    $cuenta = $fn155->fn155_uusuario_xest($id, $estado);
    if ($cuenta==0) {
        ?>
...
if ($opc_cn == 5) {
    ...
    if($pass1 == $pass2){
        $cuenta = $fn155->fn155_uusuario_xpass($id, $clave);
        if($cuenta == 1){
...
if ($opc_cn == 6) {
    $id = $_POST['dato_1'];
    $idrol = $_POST['dato_2'];
    $cuenta = $fn155->fn155_uusuario_xrol($id, $idrol);
    $include = 1;
}
    ...
    <table id="example3" class="display">
        ...
        <tbody>
            ...
            while ($menu = $tabla->fetch_assoc()) {
                ...
                $estado = $fn155->fn155_estado_xid($st);
                $rolactivo = $fn155->fn155_rusuario_xrol($idrol);
                $roles = $fn155->fn155_rrol_all();
                ?>
                <tr>
                    ...
                    <td>
                        <div id="fill_<?php echo $id ?>">
                            <div class="custom-control custom-checkbox mb-3">
                                ...
                                if ($st == 0) {
                                    ?>
                                    <input value="1" name="estado" onchange="cn155_f4(4, <?php echo $id ?>, this.value)" type="checkbox" class="custom-control-input" id="customCheckBox<?php echo $id ?>" required>
                                    <label class="custom-control-label" for="customCheckBox<?php echo $id ?>">Inactivo</label>
                                    <?php
                                } else if ($st == 1) {
                                    ?>
                                    <input value="0" name="estado" onchange="cn155_f4(4, <?php echo $id ?>, this.value)" checked type="checkbox" class="custom-control-input" id="customCheckBox<?php echo $id ?>" required>
                    ...
                    <td>
                        <select onchange="cn155_f6(6,<?php echo $id ?>,this.value)">
                    ...
                    <td>
                        <div class="d-flex">
                            <button onclick="md155_d2(2,<?php echo $id ?>)" data-bs-toggle="modal" data-bs-target="#modalcontent_md" class="btn btn-warning shadow btn-xs sharp mr-1"><i class="fa fa-pencil-alt"></i></button>
                            ...
                            <button onclick="md155_d5(5,<?php echo $id ?>,'<?php echo $mail ?>')" data-bs-toggle="modal" data-bs-target="#modalcontent_sm" class="btn btn-warning shadow btn-xs sharp mr-1"><i class="fa fa-key"></i></button>
                        </div>												
                    </td>												
                </tr>
                <?php
