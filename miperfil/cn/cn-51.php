...
//    echo $fecha_ingreso.'--'.$id_concon.'--'.$id_usuvivienda.'--'.$valor_ingreso.'--'.$repeat.'--'.$mora.'--'.$firstmonth.'--'.$valor_firstmonth;
    if($tipo_concepto==1){
        if($id_concon != 0 && $monto_trans != ''){
            $cuenta = $a->fn51_ctrasanccion_xdata($idconj_open, $id_concon, $id_usuvivienda, $fecha_trans,
        ...
        if($id_concon != 0 && $monto_trans != '' && $repeat > 0 && $mora >= 0 ){
            ...
            $sql = substr($sql, 0, -1);
            $cuentingreso = $a->fn51_ctrasanccion2_xdata($sql);
            if($cuentingreso > 0){
                echo $fnalert->fnalert_save(1);
            } else {
                echo $fnalert->fnalert_save(7);
...
if ($opc_cn == 2) {
    ...
    if($documento_trans != '' && $monto_trans != ''){
        $veriffactura= $a->fn51_rtransaccion_xdata($documento_trans,$id_trans); 
        if(count($veriffactura)==0){
            $cuenta = $a->fn51_utransaccion_xdata($documento_trans, $monto_trans, $oberva_trans, $id_trans);
            if($cuenta==1){
...
if ($opc_cn == 8) {
    ...
    <?php 
    $conseptoegresos = $a->fn51_rconceptos_xtipo($tipo_concepto,$idconj_open);
...
if ($opc_cn == 9) {
    ...
    if (move_uploaded_file($_FILES['file_comprobante']['tmp_name'], $fichero_subido)) {
        $cuenta = $a->fn51_utransaccion_xid($id, $archivo);
        if($cuenta==1){
            ?>
            <script>
...
<!--                                            <td> <?php echo $txttipo ?></td>-->
                    ...
                    <td>
                        <div class="d-flex">
                            ...
                            <button onclick="cn51_c010_d1(10)"  style="margin-left: 5px" title="ENVIO DE REGISTRO"  class="btn btn-warning shadow btn-xs sharp mr-1"><i class="flaticon-381-send-1"></i></button>
                            <?php 
