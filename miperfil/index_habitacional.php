...
if (isset($_GET['opc']) && isset($_GET['conf']) && isset($_GET['dato3']) ) {
    $opc = $_GET['opc'];
    $_SESSION['sesionwisuid'] = $_GET['dato3'];
    include './sesiones/opensesion.php';
    $confirmacion = $_GET['conf'];
    $idhabrecidencial = $_GET['dato3'];
    $detopc = $m->fn_rmenu2_xid($opc);
    $verificarhabitatd = $m->fn4_rconjunto_xid($idusu_open, $idhabrecidencial);
    
    
    if($confirmacion != md5($verificarhabitatd[0]['id_usuario'].''.$verificarhabitatd[0]['id_empresa'])){
        $opc = "-1";
